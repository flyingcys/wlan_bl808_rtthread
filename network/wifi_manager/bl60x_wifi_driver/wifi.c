...
/* Transmit packet. */
err_t bl_wifi_tx(uint8_t idx, struct pbuf* p)
{
...
#define WARNING_LIMIT_TICKS_TX_SIZE  2000
    ...
    if (idx == 0)
        wlan = &wifiMgmr.wlan_sta;
    else
        wlan = &wifiMgmr.wlan_ap;

    return bl_output(bl606a0_sta.bl_hw, NULL, p, 0 == wlan->mode, &custom_cfm);
}
#endif
...
int bl_wifi_eth_tx(struct pbuf *p, bool is_sta, struct bl_custom_tx_cfm *custom_cfm)
{
    ...
    // }
    ret = bl_output(bl606a0_sta.bl_hw,  NULL, p, is_sta, custom_cfm);
    if (ret == ERR_OK) {
        return 0;
    } else {
        return -1;
