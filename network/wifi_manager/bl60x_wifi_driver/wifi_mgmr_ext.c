...
wifi_interface_t wifi_mgmr_sta_enable(void)
{
    ...
    wifi_eth_sta_enable(wifi_mgmr_sta_netif_get(), wifiMgmr.wlan_sta.mac);
    return &(wifiMgmr.wlan_sta);
}

int wifi_mgmr_mode_set(int mode)
{
    if (mode == 0)
        wifiMgmr.wlan_sta.mode = 0;         // sta
    else
        wifiMgmr.wlan_ap.mode = 1;         // ap
}
...
int wifi_mgmr_scan_complete_callback()
{
    ...
    if (scan_cb != NULL){
        ...
        struct wifi_mgmr_scan_item *scan_buff = (struct wifi_mgmr_scan_item *)bl_os_malloc(sizeof(struct wifi_mgmr_scan_item) * total_items);
        if (scan_buff == NULL)
            return -1;
        ...
        for (i = 0; i < sizeof(wifiMgmr.scan_items)/sizeof(wifiMgmr.scan_items[0]); i++) {
            if (wifiMgmr.scan_items[i].is_used && (!wifi_mgmr_scan_item_is_timeout(&wifiMgmr, &wifiMgmr.scan_items[i]))) {
                scan_buff[total_items].channel = wifiMgmr.scan_items[i].channel;
                memcpy(scan_buff[total_items].bssid, wifiMgmr.scan_items[i].bssid, 6);
                scan_buff[total_items].rssi = wifiMgmr.scan_items[i].rssi;
                scan_buff[total_items].ppm_abs = wifiMgmr.scan_items[i].ppm_abs;
                scan_buff[total_items].ppm_rel = wifiMgmr.scan_items[i].ppm_rel;
                scan_buff[total_items].wps = wifiMgmr.scan_items[i].wps;
                scan_buff[total_items].mode = wifiMgmr.scan_items[i].mode;
                scan_buff[total_items].auth = wifiMgmr.scan_items[i].auth;
                scan_buff[total_items].cipher = wifiMgmr.scan_items[i].cipher;
                scan_buff[total_items].group_cipher = wifiMgmr.scan_items[i].group_cipher;
                memcpy(scan_buff[total_items].ssid, wifiMgmr.scan_items[i].ssid, sizeof(wifiMgmr.scan_items[i].ssid));

                total_items ++;
            }
        }

        scan_cb(total_items, (void *)scan_buff, scan_data);
        bl_os_free(scan_buff);
    }
    return 0;
}
