...
#ifdef IPC_HOST_DEBUG
#define IPC_HOST_DBG bl_os_printf
#else
#define IPC_HOST_DBG(...) do {} while(0)
#endif
...
volatile struct txdesc_host *ipc_host_txdesc_get(struct ipc_host_env_tag *env)
{
    ...
    IPC_HOST_DBG("free_idx is %u(%u), used_idx is %u(%u), cnt is %u\r\n",
        free_idx, 
    ...
    if (used_idx > free_idx) {
        IPC_HOST_DBG("[TX] used_idx case found, used:free %u:%u\r\n", used_idx, free_idx);
        used_issue++;
    }
    ...
    if (free_idx != (used_idx + nx_txdesc_cnt[0]))
    {
        ...
        if ((free_idx - used_idx) <= nx_txdesc_cnt[0]) {
            ...
            /*idx error*/
            while (1) {
                IPC_HOST_DBG("TX IPC HOST race condition\r\rn");
            }
        }
    }
    else
...
void ipc_host_irq(struct ipc_host_env_tag *env, uint32_t status)
{
    ...
    if (status & IPC_IRQ_E2A_TXCFM)
    {
        ...
        IPC_HOST_DBG("[IRQ-BH] IPC_IRQ_E2A_TXCFM\r\n");
#if 0
        ...
        for (i = 0; i < IPC_TXQUEUE_CNT; i++) {
            ...
            if (status & q_bit) {
                ...
                IPC_HOST_DBG("[IRQ-BH] CFM handler\r\n");
