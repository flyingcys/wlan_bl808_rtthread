...
#define mgmr_TASK_PRIORITY     (8)
#define mgmr_TASK_STACK_SIZE    (4096)
...
static bool stateGlobalGuard_fw_scan(void *ch, struct event *event)
{
    ...
    if (&stateIdle != wifiMgmr.m.currentState &&
            ...
            bl_os_event_notify(CODE_WIFI_ON_SCAN_DONE, WIFI_SCAN_DONE_EVENT_BUSY);
            return false;
    }
...
static void stateConnectingEnter( void *stateData, struct event *event )
{
   ...
   bl_os_event_notify(CODE_WIFI_ON_CONNECTING, 0);
}
...
//     netifapi_netif_set_link_up(wifi_mgmr_ap_netif_get());
// void dhcpd_start(struct netif *netif);
//     if (ap->use_dhcp_server) {
//         netifapi_netif_common(wifi_mgmr_ap_netif_get(), dhcpd_start, NULL);
//     }
    ...
    // }
    bl_os_event_notify(CODE_WIFI_ON_AP_STARTED, 0);
...
//     netifapi_netif_common(wifi_mgmr_ap_netif_get(), NULL, dhcp_server_stop);
    bl_os_printf(DEBUG_HEADER "Removing ETH interface ...\r\n");
    // netifapi_netif_remove(wifi_mgmr_ap_netif_get());
    wifiMgmr.inf_ap_enabled = 0;
    bl_os_event_notify(CODE_WIFI_ON_AP_STOPPED, 0);
    ...
    if (msg->data1) {
        ...
        if (&stateConnectedIPYes == wifiMgmr.m.currentState) {
            ...
            if (auto_repeat) {
                ...
                auto_repeat = 1;
                bl_main_denoise(1);
                bl_os_event_notify(CODE_WIFI_ON_MGMR_DENOISE, 0);
            }
        }
    } else {
        //disable denoise
...
#endif
    bl_os_event_notify(CODE_WIFI_ON_CONNECTED, 0);
}
...
static void stateConnectedIPYes_enter( void *stateData, struct event *event )
{
    ...
    bl_os_printf("Entering %s state, up time is %.1fs, cost time is %.1fs\r\n", (char *)stateData, now/1000.0, (now - wifiMgmr.connect_time)/1000.0);
#endif
    bl_os_event_notify(CODE_WIFI_ON_GOT_IP, 0);
      ...
      void *newStateData )
{
    ...
    bl_os_event_notify(CODE_WIFI_CMD_RECONNECT, 0);
}
...
static void stateDisconnect_enter(void *stateData, struct event *event)
{
    ...
    bl_os_event_notify(CODE_WIFI_ON_DISCONNECT, wifiMgmr.wifi_mgmr_stat_info.status_code);

#ifdef CFG_CHIP_BL602
    if (0 == bl60x_check_mac_status(&is_ok) && 0 == is_ok) {
        bl_os_event_notify(CODE_WIFI_ON_EMERGENCY_MAC, 0);
...
void wifi_mgmr_start(void)
{
    ...
    bl_os_event_notify(CODE_WIFI_ON_MGMR_DONE, 0);
...
void wifi_mgmr_start_background(wifi_conf_t *conf)
{
    ...
    bl_os_task_create("wifi_mgmr", _wifi_mgmr_entry, mgmr_TASK_STACK_SIZE, NULL, mgmr_TASK_PRIORITY, NULL);
}
