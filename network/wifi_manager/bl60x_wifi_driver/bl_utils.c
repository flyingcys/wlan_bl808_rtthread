...
#ifdef BL_UTILS_DEBUG
#define UTILS_DBG       bl_os_printf
#else
#define UTILS_DBG(...) do {} while(0)
#endif
...
static void bl_rx_mgmt(uint32_t *skb,  struct hw_rxhdr *hw_rxhdr, int len, bl_rx_info_t *info)
{
	...
	if (ieee80211_is_beacon(mgmt->frame_control)) {
#if 0
        UTILS_DBG("[RX] %04X BCN Received, freq %u, rssi %d\r\n",
                mgmt->frame_control,
        ...
        if (ieee80211_is_deauth(mgmt->frame_control)) {
            UTILS_DBG("[RX] Deauth Received\r\n");
        } else if (ieee80211_is_disassoc(mgmt->frame_control)) {
            UTILS_DBG("[RX] Disassoc Received\r\n");
        }
    } else if ((ieee80211_is_action(mgmt->frame_control) &&
                (mgmt->u.action.category == 6))) {
        UTILS_DBG("[RX] FT EVENT\r\n");
    } else if (ieee80211_is_data(mgmt->frame_control)){
        UTILS_DBG("[RX] %08X %04X, DATA SA: %02X:%02X:%02X:%02X:%02X:%02X DA: %02X:%02X:%02X:%02X:%02X:%02X\r\n",
                (unsigned int)counter++,
                mgmt->frame_control,
                mgmt->sa[0],
                mgmt->sa[1],
        ...
        UTILS_DBG("[RX] Upload MGMT Frame? %04X\r\n", mgmt->frame_control);
    }
}
...
int tcpip_stack_input(void *swdesc, uint8_t status, void *hwhdr, unsigned int msdu_offset, struct wifi_pkt *pkt, uint8_t extra_status)
{
    ...
    if (!sniffer) {
        ...
        UTILS_DBG("********************ETH Start******************************\r\n");
        UTILS_DBG("  Eth Dst " MAC_FMT "\r\n", MAC_LIST(hdr->h_dest));
        UTILS_DBG("  Eth Src " MAC_FMT "\r\n", MAC_LIST(hdr->h_source));
        UTILS_DBG("  Eth Proto %04X, %p:%p, len %u\r\n", hdr->h_proto, bl_vif, bl_vif ? bl_vif->dev : NULL, hw_rxhdr->hwvect.len);
        UTILS_DBG("********************ETH End******************************\r\n");
    ...
    if (sniffer) {
        ...
        extern int bl_wifi_rx(uint8_t idx, struct pbuf *p);
        uint8_t mode;
        if (bl_vif->dev == &wifiMgmr.wlan_sta.netif)
            mode = 0;
        else
            mode = 1;

        if (bl_wifi_rx(mode, h) != 0)
        {
            UTILS_DBG("===============bl_wifi_rx error!=========\r\n");
            pbuf_free(h);
        }
    }
