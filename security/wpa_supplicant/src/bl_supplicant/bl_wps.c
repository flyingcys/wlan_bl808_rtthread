...
{
    rt_sem_t scan_sem = (rt_sem_t)data;
    rt_sem_release(scan_sem);
}
...
static wps_scan_result_t wps_scan_(void)
{
    ...
    rt_sem_t scan_sem = NULL;
    scan_sem = rt_sem_create("scan_sem", 0, RT_IPC_FLAG_PRIO);
    if (scan_sem != NULL)
    {
    ...
    while (rt_tick_get() - sm->start_tick < rt_tick_from_millisecond(WPS_TIMEOUT_MS)) {
        wifi_mgmr_scan((void *)scan_sem, wps_scan_complete_);
        rt_sem_take(scan_sem, RT_WAITING_FOREVER);
...
static void wifi_event_cb_(input_event_t *event, void *private_data)
{
    ...
    if (event->code == CODE_WIFI_ON_DISCONNECT) {
        ...
        rt_mq_send(sm->event_queue, &msg, sizeof(wps_evq_msg_t));
    }
}

static void wps_timeout_cb_(void *param)
{
    struct wps_sm *sm = gWpsSm;
...
static void wps_success_timer_cb_(void *param)
{
...
static void wps_task_(void *pvParameters)
{
    ...
    rt_tick_t time_elapsed = rt_tick_get() - sm->start_tick;
    if (time_elapsed >= WPS_TIMEOUT_MS) {
        notify_user_(BL_WPS_EVENT_TIMEOUT, 0);
        goto error_out;
    }
    rt_tick_t timeout_timer_period = rt_tick_from_millisecond(WPS_TIMEOUT_MS) - time_elapsed;
    sm->timeout_timer = rt_timer_create("wps timeout", wps_timeout_cb_, NULL, timeout_timer_period, RT_TIMER_FLAG_ONE_SHOT | RT_TIMER_FLAG_HARD_TIMER);
    if (sm->timeout_timer != NULL)
    {
        rt_timer_start(sm->timeout_timer);
    }

    for (;;) {
        wps_evq_msg_t msg;
        bool exit_loop = false;
        if (rt_mq_recv(sm->event_queue, &msg, sizeof(wps_evq_event_t), RT_WAITING_FOREVER) < 0) {
            break;
        }

        switch (msg.event) {
        case WPS_EVQ_SUCCESS:
            sm->success_cb_timer = rt_timer_create("wps success", wps_success_timer_cb_, NULL, rt_tick_from_millisecond(1000), RT_TIMER_FLAG_ONE_SHOT | RT_TIMER_FLAG_HARD_TIMER);
            if (sm->success_cb_timer != NULL)
            {
                rt_timer_start(sm->success_cb_timer);
            }
            break;
        ...
        case WPS_EVQ_DISCONNECTED:
            if (sm->timeout_timer) {
                if (sm->timeout_timer->parent.flag & RT_TIMER_FLAG_ACTIVATED)
                {
                    rt_timer_stop(sm->timeout_timer);
                }
                
                rt_timer_delete(sm->timeout_timer);
                sm->timeout_timer = NULL;
            }
            if (sm->success_cb_timer) {
                if (sm->success_cb_timer->parent.flag & RT_TIMER_FLAG_ACTIVATED)
                {
                    rt_timer_stop(sm->success_cb_timer);
                }
                rt_timer_delete(sm->success_cb_timer);
...
bl_wps_err_t bl_wifi_wps_start(const struct bl_wps_config *config)
{
    ...
    sm->event_queue = rt_mq_create("event_queue", sizeof(wps_evq_msg_t), WPS_EVENT_QUEUE_CAPACITY, RT_IPC_FLAG_PRIO);
    if (sm->event_queue == NULL)
    {
        ret = BL_WPS_ERR_MEMORY;
        goto ret;
    }
    ...
    sm->wps_task_hdl = rt_thread_create("wps", wps_task_, NULL, WPS_TASK_STACK_DEPTH * 4, 21, 20);
    if (sm->wps_task_hdl != NULL)
    {
        rt_thread_startup(sm->wps_task_hdl);
    }
    else
    {
        ret = BL_WPS_ERR_MEMORY;
